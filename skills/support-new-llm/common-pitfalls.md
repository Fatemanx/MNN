...
如果某个模型是否开启 `enable_thinking` 取决于 chat template 结构，判断时要基于 **tokenizer 里的特殊 token** 和 **template 的实际内容**，不要依赖本地路径名、软链接名或目录是否包含模型关键字。

同时要注意：从 tokenizer 读出的 template 通常已经是**真实换行**字符串，匹配 `<think>` 空块时应统一换行后再查找 `'<think>\n\n</think>\n\n'`，不要写成字面量 `'\\n'`，否则很容易出现只在某些导出路径下静默漏判的 false negative。
...
## 10. 先判断“架构支持”还是“模板语义支持”

### 问题描述

有些新模型虽然名字和已支持模型家族接近，但真正缺的并不是新的 `model_mapper` / `Decoder` / `Attention` 实现，而只是 **chat template / thinking 开关 / stop token / prompt cache** 这类外围语义支持。

典型信号：

- `config.json` 的 `model_type` 仍是现有通路（如 `llama`）
- `architectures` 仍是标准 HF 类（如 `LlamaForCausalLM`）
- 权重路径、attention 投影名、decoder 子模块名都与默认映射一致
- README 重点强调 `/think`、`/no_think`、特殊模板 token，而不是新的层结构

### 正确做法

- **先按步骤 1 的 5 个架构问题判定是否真的需要新映射。**
- 如果主干结构完全匹配现有模型，不要急着注册新的 `model_type`，先检查：
  - `llmexport.py` 是否需要补 `jinja.context`
  - `tokenizer.py` 是否需要补 stop token
  - `prompt_cache_utils.hpp` 是否需要补新的思考标签清理
  - C++ chat template 测试是否覆盖 `enable_thinking` 等上下文开关

### 最小验证

在新增 `model_mapper` 之前，先做两件事：

1. 用当前 `model_type` 跑 `LlmModel.from_pretrained()`，确认能否直接加载
2. 用一个最小 chat template 测试验证 `enable_thinking` / 特殊 thought token 是否是唯一缺口

如果这两步都成立，优先修外围语义，不要把问题误判成“新架构”。

---

## 11. Jinja 单测里显式构造 `json::object()`

### 问题描述

在 C++ 的 minja / tokenizer 单测里，如果用 brace-init 直接 `push_back({{"role", "user"}, {"content", "hi"}})`，不同 JSON 实现/重载下可能不会构造成你以为的 object，导致 `message.role`、`message.content` 在模板里变成 undefined，而表面现象只是模板“只渲染了 generation prompt”。
...
如果模板里的 `{% if message.role == "user" %}` / `{% if message.role == "assistant" %}` 全都像没生效一样，先不要怀疑模板逻辑，先检查测试数据本身是不是 object。
...
## 15. 流式首段清理要按 llm_demo 增量路径验证

### 问题描述

`llm_demo` 在指定 `max_token_number > 0` 时，不是一次性走完整 `response(prompt)`，而是先调用 `response(..., max_new_tokens=0)` 做 prefill，再循环调用 `generate(1)` 逐 token 解码。和思维标签、前缀缓冲、首段清理相关的逻辑，如果只验证单次 `response(prompt)`，可能会漏掉这条真实常用路径上的问题。

### 解决方案

- 修改流式输出逻辑后，同时验证：
  - `llm->response(prompt)` 单次调用路径
  - `llm_demo` 的 `response(..., 0) + generate(1)` 增量路径
- 尤其要检查首 token/首段缓冲状态是否在 `generate_init()` 后跨多次 `generate(1)` 保持正确。

---

## 16. 思维标签清理先做前缀级最小修复，不要全局删 token

### 问题描述

带 reasoning 模板的模型在 `enable_thinking=false` 或模板切换时，常见问题是**回复开头**残留一个孤立的 `</think>` / `<|thought_end|>`，或者出现空的 `<think></think>` 块。这个问题很容易被“全局删除 think 标签 token”快速掩盖，但那会改变正文语义，误删模型本来想原样输出的标签文本。

### 解决方案

- 流式清理优先做成**前缀专用**：
  - 只处理回复开头的 orphan closer
  - 只处理回复开头的空 think block
- 如果服务端需要隐藏 reasoning 内容，优先做成**前缀级 reasoning block 剥离**，不要把正文里任意位置的 think block 一起删掉。

### 验证标准

- `disable_thinking` 路径下，真实 `llm_demo` 增量输出不再出现前缀 stray closer。
- 单测必须覆盖：
  - leading `</think>` / `<|thought_end|>`
  - leading empty `<think></think>` / `<|thought_begin|><|thought_end|>`
  - 非空 think block 不应被这条流式清理逻辑删除
  - 正文中的字面量 think 标签不应被误删

---

## 17. 修复流式结束符时，要排查所有 decoding 策略是否绕过统一收尾

### 问题描述

给 `llm.cpp` 增加输出缓冲或思维标签清理后，`emitEndWith()` 往往会承担“先 flush 缓冲、再输出结束符”的统一语义。但 speculative decoding 不只一条路径，`generate.cpp`、`lookahead.cpp`、`mtp.cpp`、`dflash.cpp`、`eagle.cpp` 可能分别在 stop-token 分支里直接写 `end_with`。如果只修主路径，其他策略可能会把缓冲残留写到结束符后面，形成输出错序。

### 解决方案

- 任何涉及流式输出收尾的修复，都要全文检索：
  - `end_with`
  - `emitEndWith`
  - `is_stop(`
- 对每条 decoding 策略逐个确认：
  - stop-token 分支是否统一走 `emitEndWith()`
  - 是否仍存在直接 `*os << end_with` 的旁路写法
- 如果某条策略故意不走统一路径，必须在代码里写清理由，不能靠“这条路径很少用”默认放过。

### 验证标准

- 至少完成一次真实模型的流式回归，确认 stop-token 收尾没有把 held-back 输出写到 `end_with` 之后。
- 修改后重新 grep 一遍目标目录，确认没有遗漏的直接 `end_with` stop 分支。
